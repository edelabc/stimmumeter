...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE table_name = 'menu_items' AND column_name = 'required_role'
  ) THEN
    ALTER TABLE menu_items ADD COLUMN required_role text DEFAULT 'public' CHECK (required_role IN ('public', 'user', 'admin'));
  END IF;
END $$;
